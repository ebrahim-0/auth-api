...
const getEnv = (key: string, defaultValue?: string): string => {
  ...
  if (value === undefined && defaultValue === undefined) {
    throw new Error(
      `Environment variable ${key} is required but not set and shown as ${value} ${JSON.stringify(process.env)}`,
    );
  }
...
const getEnvNumber = (key: string, defaultValue?: number): number => {
  ...
  if (!value && defaultValue === undefined) {
    throw new Error(
      `Environment variable ${key} is required but not set and shown as ${value}`,
    );
  }
...
export const env: EnvConfig = {
  ...
  MONGODB_TEST_URI: getEnv(
    "MONGODB_TEST_URI",
    "mongodb://localhost:27017/auth-api-test",
  ),
